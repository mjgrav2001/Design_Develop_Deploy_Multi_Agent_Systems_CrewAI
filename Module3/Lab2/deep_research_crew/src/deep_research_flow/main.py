...
from deep_research_flow.patch import disable_ssl_verification
...
class DeepResearchFlow(Flow[ResearchState]):
    ...
    def analyze_query(self):
        ...
        prompt = (# Write the prompt for the LLM to decide if the query is simple or requires research 
                  "Decide whether the query is simple or requires research. If a complex query detected is detected, return as final response 'RESEARCH' else return the response 'SIMPLE'."
    ...
    def simple_answer(self):
        ...
        prompt = (# Write the missing part of the query for the LLM 
                 "Return a simple response for the query in form of a string.
                 f"Query: \"{self.state.user_query}\"\n\n"
                 "Answer:"
                 )
        # set up the LLM
